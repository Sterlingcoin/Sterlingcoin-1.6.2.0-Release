...
const QString apiBittrexTrades = "http://bittrex.com/api/v1.1/public/getmarkethistory?market=BTC-SLG&count=100";
...
void PoolBrowser::on_btnConvertSterlingcoin_clicked() {
    double SterlingcoinQty = ui->txtConvertSterlingcoinQty->text().toDouble();
    double totalBtc = _bittrexMarketSummary->getLastCurrent(double()) * SterlingcoinQty;
    double totalGbp = totalBtc * _dBtcPriceCurrent;

    ui->lblConvertSterlingcoinResults->setText("£" + QString::number(totalGbp, 'f', 2) +
...
{
    QDesktopServices::openUrl(QUrl("https://www.bittrex.com/Market/Index?MarketName=BTC-SLG"));
}
void PoolBrowser::openCryptsy()
{
    QDesktopServices::openUrl(QUrl("https://www.cryptsy.com/markets/view/304"));
}
...
void PoolBrowser::bittrexMarketSummary(QNetworkReply* response)
{
    ...
    for(int i = 0; i < qslApiResponse.count(); i++){
        ...
        if(read_string(qslApiResponse[i].toStdString(), jsonResponse)) {
            ...
            if (getPairValue(jsonObject, "MarketName").get_str() == "BTC-SLG") {
                try {
    ...
    updateLabel(ui->lblBittrexVolumeGbp,
